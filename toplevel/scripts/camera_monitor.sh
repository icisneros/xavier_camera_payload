...
while true; do
  ...
  output=$(tmux capture-pane -p -t $SESSION:$WINDOW.$pane_id)
  ...
  if [ $has_stopped -eq 1 ]; then
    ...
    tmux send-keys -t $SESSION:$WINDOW.$cam_rec_pane_id C-b C-c
    tmux send-keys -t $SESSION:$WINDOW.$cam_rec_pane_id C-b C-c
    sleep 20   # very conservative wait time for command to stop


    # camera system restart
    tmux send-keys -t $SESSION:$WINDOW.$pane_id "sudo rm -rf /tmp/image_servers/ /dev/shm/sem.image.sem" Enter
    sleep 3
    
    # Search for a password prompt in the output
    output=$(tmux capture-pane -p -t $SESSION:$WINDOW.$pane_id)
    if [[ $output == *"[sudo] password for airlab:"* ]]; then
      tmux send-keys -t $SESSION:$WINDOW.$pane_id "passme24" Enter
    else
      echo "Pane is not requesting a password"
    fi

    tmux send-keys -t $SESSION:$WINDOW.$pane_id "sudo systemctl restart nvargus-daemon.service" Enter
    ...
    # Run camera
    tmux send-keys -t $SESSION:$WINDOW.$pane_id "roslaunch nvidia_to_ros argus_camera.launch" Enter
    ...
    tmux send-keys -t $SESSION:$WINDOW.$cam_rec_pane_id "roslaunch nvidia_to_ros field_record_camera.launch" Enter
