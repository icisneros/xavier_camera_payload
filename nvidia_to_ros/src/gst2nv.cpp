  ...
  if (sample)
    {
      ...
      ros::Time current_stamp = start_time + ros::Duration(buffer->dts/1000000000, buffer->dts%1000000000);
      nv2ros::NvImageMessage img_msg(dmabuf_fd, buf_params, buf_params_ex, current_stamp);
      pub->publish(img_msg);
...
ros::Time get_start_time(std::string filename){
  ...
  while(getline(ss, tokens.back(), '_'))
    tokens.push_back("");
  
  return ros::Time(std::atoi(tokens[tokens.size()-2].c_str()),
		   std::atoi(tokens[tokens.size()-1].c_str()));
}
